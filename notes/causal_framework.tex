...
\noindent When the target population is the RWD ($s = 0$), the survival-scale estimands become
\begin{align}
...
\begin{remark}[Causal estimands in the target population]
  In equations~\eqref{eq:rd_aft}--\eqref{eq:rmst_aft}, the confounding function $\cf(x)$ appears because the target population is the RWD ($s=0$). If instead one targets the RCT population ($s=1$), the $\cf(x)$ term drops out and the estimands depend only on $m_0(x,1)$ and $\CATE(x)$.
\end{remark}

\begin{remark}[Causal interpretation of the survival estimands]
  The risk difference~\eqref{eq:rd_aft} and RMST difference~\eqref{eq:rmst_aft} involving $\cf(x)$ reflect the \emph{causal} treatment effect in the RWD population. The confounding function enters through the survival curve under treatment, $S_1$, because the observed outcomes of the treated group in the RWD are distorted by unmeasured confounding. The decomposition~\eqref{eq:individual} ensures that $\CATE(x)$ captures the true causal effect while $\cf(x)$ absorbs the bias, so that the survival contrasts computed from~\eqref{eq:surv1}--\eqref{eq:surv0} with $\cf$ removed yield the correct causal comparison.
\end{remark}
