...
\begin{enumerate}[label=(G\arabic*),leftmargin=*]
  ...
  \item identifiable extrapolation of causal survival contrasts beyond the
        RCT follow-up horizon $t_{\RCT}$, using the longer follow-up of the OS.
\end{enumerate}
...
\begin{itemize}[leftmargin=*]
  ...
  \item the OS identifies the \emph{shape} of the log-time distribution at
...
The HDP prior on $\varepsilon$ \emph{is the mechanism} by which OS follow-up
informs the tail: the shared atoms accumulate mass where both sources have
data; OS-only mass shapes the tail; the hierarchical structure prevents
...
arms in the OS but long-term survival does not --- a single $\cf(\X)$ is
mis-specified. Worth flagging, with sensitivity to time-varying confounding.

\paragraph{Challenge 3: period and calendar-time effects in the OS.}
Long OS follow-up necessarily mixes historical cohorts treated under earlier
standards of care. The patients contributing to the OS tail are
...
condition on calendar year as a covariate; restrict the OS window;
or model an explicit period effect alongside $\gf$.

\paragraph{Challenge 4: informative censoring in the OS.}
Trial censoring is largely administrative and credibly non-informative;
OS censoring includes loss-to-follow-up that may correlate with
...
With a fully pooled DP, the OS would dominate the error distribution
...
From Proposition~3(iii) of the framework draft, the OS alone identifies
$\tauf + \cf$, not the two separately; the separation is anchored by the
RCT. With a small RCT and a large OS, the prior on $\cf$ does substantial
...
exploring via simulations that vary $n_{\RCT}/n_{\OS}$.
...
required: the OS must contain survivors at long $t$ \emph{with covariate
profiles matching the RCT}. If the OS long-followup is dominated by, say,
...
Under (G1), the natural target is the OS population (representativeness
...
only when the target is the OS.
...
\begin{enumerate}[label=(E\arabic*), leftmargin=*]
  ...
  \item \textbf{Relative-survival / additive-hazards formulation.}
        ...
        tables), so OS-tail information competes against a hard external
  ...
  \item \textbf{Sensitivity analysis on unmeasured confounding.}
        ...
        $\cf$ consistent with observed RCT-OS discrepancy.
  ...
  \item \textbf{Posterior projection summaries for the HTE.}
        ...
        how those subgroup summaries shift when the OS is included.
