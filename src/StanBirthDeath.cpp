                    ...
                    RoutingMap& routing_map, int irs_mode)
{
  ...
  if (random.uniform() < prob_birth) {
    ...
    if (irs_mode == 2) {
      // Draw-then-decide: draw routing for NaN obs, include ALL in stats.
      DrawRoutingAndGetSufficientStatistics(
        tree, target_leaf, split_var, cut_val,
        cutpoints, data_info, sigma, prior_info.eta,
        left_count, left_sum, right_count, right_sum,
    ...
    // ----------------------------------------------------------------
    // Death proposal (same for both modes)
    // ----------------------------------------------------------------
    StanTree* nog_node           = nullptr;
    double    log_proposal_ratio = 0.0;
    ...
    // Sufficient statistics using routing map.
    size_t left_count = 0, right_count = 0;
    double left_sum   = 0.0, right_sum = 0.0;
    GetSufficientStatistics(tree, nog_node->GetLeft(), nog_node->GetRight(),
                            cutpoints, data_info,
    ...
    if (std::log(random.uniform()) < log_alpha) {
      ...
      // Remove routing indicators for the dying node before pruning.
      RemoveRoutingIndicators(nog_node, routing_map);
