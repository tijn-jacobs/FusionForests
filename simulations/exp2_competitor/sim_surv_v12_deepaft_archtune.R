# ──────────────────────────────────────────────────────────────────────────────
# deepAFT competitor with the HIDDEN-LAYER ARCHITECTURE cross-validated.
#
# Same experiment as sim_surv_v12_deepaft.R (right-censored RWD; S-/T-learners on
# the RCT alone and on the RCT+RWD pool), but the tuning searches over the hidden-
# layer sizes as well as the optimiser hyperparameters.  We do NOT use dnn's
# hyperTuning(node = TRUE): it randomises node counts during CV but only ever
# returns the best *control*, never the best architecture.  Instead we run our
# own 10-fold CV random search over (architecture, lr, momentum, L2 penalty),
# scored by the C-index, and keep the best of both.
#
# HPC format (foreach over replications; output to $TMPDIR).  The tuning itself is
# parallelised over the random draws via the same backend.
# ──────────────────────────────────────────────────────────────────────────────
...
# ── deepAFT competitors (S- and T-learner), architecture cross-validated ──────
# predict.deepAFT returns $predictors = the AFT log-time linear predictor mu.
get_lp <- function(pr) {
  if (is.list(pr) && !is.null(pr$predictors)) return(as.numeric(pr$predictors))
  as.numeric(pr)
}

# Candidate hidden-layer architectures to search over (the output neuron is
# appended automatically).  The first, c(8, 6), is the deepAFT paper's choice;
# the others "try a bit more" -- wider layers and a deeper three-layer net.
arch_grid <- list(c(8L, 6L), c(16L, 8L), c(16L, 16L),
                  c(32L, 16L), c(32L, 16L, 8L), c(64L, 32L))

# A dNNmodel with ReLU hidden layers and an identity output for `hidden` nodes.
build_model <- function(hidden, p) {
  dNNmodel(units       = c(hidden, 1L),
           activation  = c(rep("relu", length(hidden)), "idu"),
           input_shape = p)
}

# K-fold CV C-index for one (hidden, control) configuration on (x, Surv y).
cv_cindex <- function(x, y, hidden, control, folds) {
  vapply(sort(unique(folds)), function(k) {
    tr <- folds != k; te <- folds == k
    fit <- tryCatch(deepAFT(x[tr, , drop = FALSE], y[tr],
                            model = build_model(hidden, ncol(x)), control = control),
                    error = function(e) NULL)
    if (is.null(fit)) return(NA_real_)
    pr <- tryCatch(predict(fit, newdata = x[te, , drop = FALSE], newy = y[te]),
                   error = function(e) NULL)
    if (is.null(pr) || is.null(pr$c.index)) NA_real_ else as.numeric(pr$c.index)
  }, numeric(1))
}

# Random search over architecture + (lr, momentum, L2), 10-fold CV on the C-index.
# Returns the best configuration: hidden units, activations, dnnControl and score.
# The R random draws are evaluated in parallel via the registered backend.
tune_deepaft <- function(x, time, status, K = 10L, R = 60L,
                         lr = c(1e-4, 3e-3), alpha = c(0.5, 0.97),
                         lambda = c(0, 10), seed = 1L) {
  x <- as.matrix(x); y <- Surv(time, status)
  set.seed(seed)
  folds <- sample(rep_len(seq_len(K), nrow(x)))
  # Pre-draw the random configurations (reproducible, independent of run order).
  draws <- lapply(seq_len(R), function(r) {
    set.seed(seed * 1000L + r)
    list(hidden  = arch_grid[[sample.int(length(arch_grid), 1)]],
         control = dnnControl(epochs = 300, batch_size = 64, epsilon = 1e-2,
                              lr_rate = runif(1, lr[1], lr[2]),
                              alpha   = runif(1, alpha[1], alpha[2]),
                              lambda  = runif(1, lambda[1], lambda[2])))
  })
  scores <- foreach(d = draws, .combine = c,
                    .packages = c("dnn", "survival")) %dopar% {
    mean(cv_cindex(x, y, d$hidden, d$control, folds), na.rm = TRUE)
  }
  best <- if (all(is.na(scores))) 1L else which.max(scores)
  list(units   = c(draws[[best]]$hidden, 1L),
       act     = c(rep("relu", length(draws[[best]]$hidden)), "idu"),
       control = draws[[best]]$control,
       hidden  = draws[[best]]$hidden,
       score   = scores[best])
}

# Each "design" packages the right-censored training data and the evaluation
# covariates Xev for one population (RCT-only, or RCT+RWD pool with source
# indicator S; the CATE is evaluated at S = 1).
...
# Fit a deepAFT with the tuned architecture/control and return mu on newX.
deepaft_mu <- function(time, status, X, newX, tune) {
  Z   <- as.matrix(X)
  fit <- deepAFT(Z, Surv(time, status),
                 model = dNNmodel(units = tune$units, activation = tune$act,
                                  input_shape = ncol(Z)),
...
# The four estimators (signature (d, Xev, tune) so boot_cate can refit them).
fit_rct_S  <- function(d, Xev, tune) cate_S(design_rct(d, Xev),  tune)
fit_rct_T  <- function(d, Xev, tune) cate_T(design_rct(d, Xev),  tune)
fit_pool_S <- function(d, Xev, tune) cate_S(design_pool(d, Xev), tune)
fit_pool_T <- function(d, Xev, tune) cate_T(design_pool(d, Xev), tune)
...
# ── One replication: four estimators (RCT/Pooled x S/T) on one dataset ────────
run_one_sim <- function(seed, n_rct, n_rwd, ld, lu, rate_rct, rate_rwd,
  ...
  estimators <- list(
    list(name = "deepAFT-RCT-S",    fun = fit_rct_S,  tune = tune_rct),
    list(name = "deepAFT-RCT-T",    fun = fit_rct_T,  tune = tune_rct),
    list(name = "deepAFT-Pooled-S", fun = fit_pool_S, tune = tune_pool),
    list(name = "deepAFT-Pooled-T", fun = fit_pool_T, tune = tune_pool)
  )
  res <- do.call(rbind, lapply(estimators, function(est) {
    point <- safe(est$fun(d, d$X, est$tune))
    draws <- if (B_boot > 0L && !is.null(point))
      safe(boot_cate(est$fun, d, d$X, B_boot, est$tune)) else NULL
...
run_simulation_tidy <- function(n_rep, n_rct, n_rwd, ld, lu,
                                 tune_rct, tune_pool, seed_offset = 1000L) {
  cal <- calibrate(ld, lu)
  ...
  foreach(
    ...
    res <- run_one_sim(seed_offset + i, n_rct, n_rwd, ld, lu,
                       cal$rate_rct, cal$rate_rwd, tune_rct, tune_pool)
    res$lambda_d <- ld
    res$lambda_u <- lu
    res$Iter     <- i
    res
...
cat("SIMULATION (competitors): deepAFT RCT/Pooled x S/T-learner, architecture-tuned -- HPC\n")
...
n_rct   <- 150L
n_rwd   <- 350L
B_boot  <- 100L                  # bootstrap refits per fit (0 = point only)
R_tune  <- 60L                   # random architecture/hyperparameter draws

cat(sprintf("M = %d reps/cell, n_rct = %d, n_rwd = %d, B_boot = %d, R_tune = %d\n",
            M, n_rct, n_rwd, B_boot, R_tune))

grid <- expand.grid(lambda_d = c(0, 1, 2), lambda_u = c(0, 1, 2))
cat(sprintf("Grid: %d (lambda_d, lambda_u) cells\n", nrow(grid)))

# ── Tune architecture + hyperparameters once, reuse across replications ───────
# 10-fold CV random search on the (lambda_d, lambda_u) = (1, 1) dataset, separately
# for the RCT and pooled designs.  Set TUNE = FALSE to use the paper's fixed 8-6
# architecture and dnnControl() defaults instead.
TUNE <- TRUE
if (TUNE) {
  cat("\nTuning deepAFT architecture + hyperparameters (10-fold CV on C-index) ...\n")
  cal0 <- calibrate(1.0, 1.0)
  d0   <- make_data(n_rct, n_rwd, 1.0, 1.0, cal0$rate_rct, cal0$rate_rwd)
  Sp   <- c(rep(1L, d0$n_rct), rep(0L, d0$n_rwd))
  tune_rct  <- tune_deepaft(cbind(d0$X_rct, A = d0$A_rct),
                            exp(d0$y_rct), d0$st_rct, R = R_tune, seed = 1L)
  tune_pool <- tune_deepaft(
    cbind(rbind(d0$X_rct, d0$X_rwd), S = Sp, A = c(d0$A_rct, d0$A_rwd)),
    c(exp(d0$y_rct), exp(d0$y_rwd)), c(d0$st_rct, d0$st_rwd),
    R = R_tune, seed = 2L)
  cat(sprintf("  RCT-only : hidden = (%s), lr = %.2g, alpha = %.2g, lambda = %.2g, CV C = %.3f\n",
              paste(tune_rct$hidden, collapse = ", "), tune_rct$control$lr_rate,
              tune_rct$control$alpha, tune_rct$control$lambda, tune_rct$score))
  cat(sprintf("  Pooled   : hidden = (%s), lr = %.2g, alpha = %.2g, lambda = %.2g, CV C = %.3f\n",
              paste(tune_pool$hidden, collapse = ", "), tune_pool$control$lr_rate,
              tune_pool$control$alpha, tune_pool$control$lambda, tune_pool$score))
} else {
  fixed <- list(units = c(8L, 6L, 1L), act = c("relu", "relu", "idu"),
                control = dnnControl(), hidden = c(8L, 6L), score = NA_real_)
  tune_rct <- tune_pool <- fixed
}
...
for (g in seq_len(nrow(grid))) {
  ...
  all_cells[[g]] <- run_simulation_tidy(
    ...
    ld = ld, lu = lu, tune_rct = tune_rct, tune_pool = tune_pool,
    seed_offset = 1000L * g)
}
...
output_file <- file.path(Sys.getenv("TMPDIR"),
                         "sim_surv_v12_deepaft_archtune_output.rds")
