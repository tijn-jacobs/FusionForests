...
# SURVIVAL / RIGHT-CENSORED variant -- HPC parallel Monte Carlo driver.
#
# Identical to simulation_cont.R but the continuous outcome is now a latent
# log-survival time subject to right censoring.  Censoring times are drawn from
# an exponential distribution on the original time scale and log-transformed;
# the exponential rate is calibrated per source (once per grid cell, on a large
# pre-simulation) so that the censoring fraction is ~35% in BOTH sources.
#
...
# -----------------------
#   log T = m0(X) + (1-S) dev(X) + A tau(X) + A (1-S) conf(U) + sigma_S eps
...
#
#   Censoring: C ~ Exp(rate_S) on the time scale; observed log-time is
#   y = min(log T, log C), status = 1{log T <= log C}.  rate_S calibrated to
#   ~35% censoring per source.
#
# Estimators
# ---------------------------------------------------------------------------
#   (1) Fusion   : FusionForest on RCT + RWD, four-forest with confounding
#                  function c, error_dist = "source_dp_scale".
#   (2) RCT-only : CausalShrinkageForest on the RCT alone (right-censored).
#   (3) RWD-only : CausalShrinkageForest on the RWD alone (right-censored),
#                  ignores confounding.
# FusionForest is reserved for the multi-source fit; a single-source
# FusionForest is ill-defined, so the baselines use CausalShrinkageForest.
#
# Conventions (mirroring simulations/prior/simulation_censored_hpc.R)
# ------------------------------------------------------------------
#   * Cores read from CLI arg, e.g.  Rscript simulation_surv.R 192
#   * Parallelisation is across replications within a grid cell.
#   * Output is one tidy long data.frame saved to
#     $TMPDIR/simulation_surv_output.rds  (NAME MUST BE *_output.rds).
#
# Requires: FusionForests, doParallel, foreach.
################################################################################
...
output_file <- file.path(Sys.getenv("TMPDIR"), "simulation_surv_output.rds")
...
# Place this script and simulation_surv_output.rds in the same folder and run
...
# ──────────────────────────────────────────────────────────────────────────────
# res <- readRDS("simulations/main/simulation_surv_output.rds")
# 
