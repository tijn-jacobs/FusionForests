...
# ── Single-source fits (SurvivalBCF parameterisation) ────────────────────────
# FusionForest is reserved for the multi-source fusion fit; a single-source
# FusionForest is ill-defined.  Each baseline is a single-source causal forest
# in the ShrinkageTrees::SurvivalBCF parameterisation: a half-Cauchy control
# forest (200 trees, power 2, base 0.95) and a "standard" treatment forest
# (50 trees, power 3, base 0.25).  SurvivalBCF sets the treatment leaf scale to
# local_hp_treat = 1 / (sd_time * sqrt(n_treat) * 0.674); the data here are pre-
# standardised to unit variance (sigma_hat = 1 in the temp fits), so sd_time = 1
# and the scale reduces to 1 / (sqrt(n_treat) * 0.674).  The RCT is right-
# censored; the RWD is interval-censored.  Both predict the CATE on the combined
# covariate set Xev so that metrics can be split by population.
n_trees_bcf_control <- 200L
n_trees_bcf_treat   <- 50L
bcf_local_hp_treat  <- 1 / (sqrt(n_trees_bcf_treat) * 0.674)  # sd_time = 1
...
fit_single_rc <- function(y, status, X, A, Xev, N_post, N_burn) {
  CausalShrinkageForest_temp(
    ...
    local_hp_treat            = bcf_local_hp_treat,
    number_of_trees_control   = n_trees_bcf_control,
    number_of_trees_treat     = n_trees_bcf_treat,
    power_control = 2,
    power_treat = 3,
    base_control = 0.95,
    base_treat = 0.25,
...
cat("SIMULATION: FusionForest vs RCT-only vs RWD-only (SURVIVAL v11, SurvivalBCF baselines) -- HPC\n")
...
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_surv_v11_output.rds")
...
# Place this script and sim_surv_v11_output.rds in the same folder and run from the repo
...
# ──────────────────────────────────────────────────────────────────────────────
res <- readRDS("simulations/main/sim_surv_v11_output.rds")
...
# Draws to the active graphics device (e.g. the RStudio plot pane); no files
# are written.  Each call draws one 2x2 grid -- in RStudio use the plot-history
# arrows to step between the two.
plot_panel_grid <- function(dat, group, group_lab) {
...
# Plot 1: vary lambda_u, holding lambda_d = 1.  Drawn to the active device.
plot_panel_grid(a[a$lambda_d == 1, ], "lambda_u", "lambda_u")
# Plot 2: vary lambda_d, holding lambda_u = 1.  Drawn to the active device.
