...
visit_probs <- seq(0.1, 0.8, 0.1) # RWD inspection visits: deciles q10..q90
...
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_surv_v8_output.rds")
...
# Place this script and sim_surv_v8_output.rds in the same folder and run from the repo
...
# ──────────────────────────────────────────────────────────────────────────────
# res <- readRDS("simulations/main/sim_surv_v8_output.rds")
# 
...
# 
# # Boxplots over replications (combined "All" population).  Four metrics in a
# # 2x2 grid; within each panel the boxes are grouped by lambda_u from small to
# # high, with the three estimators side by side per group.
...
# # Reserve an outer bottom margin for a single shared legend.
# op <- par(mfrow = c(2, 2), oma = c(3, 0, 0, 0))
# for (metric in metrics) {
#   boxplot(a[[metric]] ~ a$method + a$lambda_u, col = cols, xaxt = "n",
#           main = metric, ylab = metric, xlab = "lambda_u", sep = " ")
#   nlu <- nlevels(a$lambda_u)
#   axis(1, at = (seq_len(nlu) - 0.5) * length(meth) + 0.5,
#        labels = levels(a$lambda_u))
...
# }
# # One common legend beneath all four panels.
