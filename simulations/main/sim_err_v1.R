...
target_cens <- 0.35              # target right-censoring fraction (RCT)
...
# ── Error settings ────────────────────────────────────────────────────────────
# Five mean-zero error laws per source, escalating in non-Gaussianity and in
# between-source discrepancy.  All generators return n mean-zero draws.
...
# `s`, recentred to mean zero.
mix_norm <- function(n, mu, w, s) {
  k <- sample.int(length(mu), n, replace = TRUE, prob = w)
  rnorm(n, mu[k], s) - sum(w * mu)
}
# Mean-zero error draw for a source ("rct"/"rwd") under one of the five settings.
draw_eps <- function(n, setting, source) {
  s <- as.character(setting)
  if (s == "1") return(rnorm(n))                                     # Gaussian, identical
  if (s == "2") return(std_t(n, 3))                                  # heavy-tailed, identical
  if (s == "3") return(if (source == "rct") rnorm(n) else std_gumbel(n))  # skew, source-specific
  if (s == "4") return(if (source == "rct") std_t(n, 5) * 0.75       # shapes + scales differ
                       else mix_norm(n, c(-0.8, 0.8), c(0.5, 0.5), 0.6) * 1.25)
  if (s == "5") return(if (source == "rct")                          # shared atoms, diff weights+scale
                         mix_norm(n, c(-1.5, 0, 1.5), c(0.2, 0.6, 0.2), 0.35)
                       else
                         mix_norm(n, c(-1.5, 0, 1.5), c(0.5, 0.1, 0.4), 0.35) * 1.3)
...
run_one_sim <- function(seed, n_rct, n_rwd, N_post, N_burn, setting,
  ...
  res <- do.call(rbind, lapply(error_priors, function(ed) {
    ...
    make_rows(ed, if (!is.null(ff)) ff$test_predictions_treat * s,
...
cat("SIMULATION: error-distribution prior (sim_err_v1): FusionForest under",
...
# Simulation settings.
M       <- 1000 #num_cores             # replications per setting (start small)
n_rct   <- 150
n_rwd   <- 350L
N_post  <- 5000L
N_burn  <- 5000L
...
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_err_v1_output.rds")
...
# Reads sim_err_v1_output.rds and prints, per (setting, method, population):
...
# ──────────────────────────────────────────────────────────────────────────────
res <- readRDS("simulations/main/sim_err_v1_output.rds")
...
grp <- cbind(rmse, bias, coverage, width, postvar) ~ setting + method + population
mean_tbl <- aggregate(grp, data = res,
                      FUN = function(x) mean(x, na.rm = TRUE), na.action = na.pass)
var_tbl  <- aggregate(bias ~ setting + method + population,
...
ord <- order(summary_tbl$setting, summary_tbl$population, summary_tbl$method)

cat("\n=== CATE metrics by error setting (mean over replications) ===\n")
print(summary_tbl[ord, c("setting", "population", "method",
...
fig_dir <- "simulations/main/figures_err"
...
plot_metrics_by_setting <- function(dat, pop = "All", file = NULL) {
  dat <- dat[dat$population == pop, ]
...
plot_metrics_by_setting(res, pop = "All",
                        file = file.path(fig_dir, "sim_err_metrics.pdf"))
