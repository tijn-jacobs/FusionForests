...
lambda_d    <- c(0.0, 1.0, 2.0)                    # grid: RWD baseline-deviation
lambda_u    <- c(0.0, 1.0, 2.0)     # grid: unmeasured confounding
sigma_rct   <- 0.75              # RCT residual SD
sigma_rwd   <- 1.25              # RWD residual SD (messier source)
target_cens <- 0.35              # target RCT right-censoring fraction
visit_probs <- seq(0.1, 0.9, 0.1) # RWD inspection visits: deciles q10..q90
factor      <- 2

# The factor is folded into each structural function, so tau(X) is the true
# CATE on the log-time scale directly.
m0   <- function(X) factor * (2 * X[, 1] - X[, 2] + 0.5 * X[, 3])
dev  <- function(X, ld) factor * ld * (X[, 4] - 0.5 * X[, 5])
tau  <- function(X) factor * (X[, 1] + 0.5 * X[, 2]^2)
conf <- function(U, lu) factor * lu * (-0.5 * U)
...
cat("SIMULATION: FusionForest vs RCT-only vs RWD-only (SURVIVAL v3) -- HPC\n")
cat("RWD interval-censored at deciles q10..q90; RCT right-censored ~35%.\n")

# Simulation settings.
M       <- 1000 #num_cores * 3L        # replications per grid cell
n_rct   <- 100L
n_rwd   <- 400L
N_post  <- 5000L
N_burn  <- 5000L
...
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_surv_v3_output.rds")
...
# Place this script and sim_surv_v3_output.rds in the same folder and run from the repo
...
# ──────────────────────────────────────────────────────────────────────────────
res <- readRDS("simulations/main/sim_surv_v3_output.rds")
