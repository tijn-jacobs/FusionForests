...
# This experiment isolates the residual model.  We set lambda_u = lambda_d = 0,
# so the two sources share the same structural mean (no unmeasured confounding,
# no between-source prognostic deviation) and differ ONLY in the distribution of
# their error term, which is set by `error_setting` (1 = easy ... 5 = hard).
# v2 pushes the error model harder: heavier, coarser censoring so a large share
# of observations enter the likelihood only through the error CDF over wide
# intervals -- where getting the residual shape/scale right (HDPM) matters most.
target_cens <- 0.50              # heavier RCT right-censoring
visit_probs <- c(0.25, 0.5, 0.75)# coarse RWD visits: quartile bins, ~25% right-censored beyond
...
# ── Error settings (v2) ───────────────────────────────────────────────────────
# Five mean-zero error laws per source.  Every non-null rung pairs a per-source
# SCALE gap (sigma_RCT << sigma_RWD, which the single-sigma gaussian/shared_dp
# priors cannot represent) with growing non-Gaussianity and source-specific
# shape -- the regime the source-specific HDP mixture is built for.  All
# generators return n mean-zero draws; the per-source sd is noted in [].
gamma_em   <- 0.5772156649
std_t      <- function(n, df) rt(n, df) * sqrt((df - 2) / df)        # variance 1
std_gumbel <- function(n) {                                          # skewed, var 1
  bs <- sqrt(6) / pi
  -rgumbel(n, loc = -bs * gamma_em, scale = bs)
}
# Normal mixture with shared atoms `mu`, weights `w`, common within-component sd
# `s`, standardised to mean zero and unit variance (so an external scale factor
# sets the per-source sd directly while the modal shape is preserved).
mix_norm <- function(n, mu, w, s) {
  mbar <- sum(w * mu)
  v    <- sum(w * ((mu - mbar)^2 + s^2))      # mixture variance
  k    <- sample.int(length(mu), n, replace = TRUE, prob = w)
  (rnorm(n, mu[k], s) - mbar) / sqrt(v)
}
# Mean-zero error draw for a source ("rct"/"rwd") under one of the five settings.
draw_eps <- function(n, setting, source) {
  s <- as.character(setting); rct <- source == "rct"
  # 1) Gaussian, identical -- null (gaussian likelihood correct).
  if (s == "1") return(rnorm(n))                                          # [1.0 / 1.0]
  # 2) Gaussian, pure scale gap -- isolates the per-source-scale advantage.
  if (s == "2") return(if (rct) rnorm(n) * 0.5 else rnorm(n) * 1.5)       # [0.5 / 1.5]
  # 3) Skew + scale gap (RCT tight normal, RWD wide skewed).
  if (s == "3") return(if (rct) rnorm(n) * 0.5 else std_gumbel(n) * 1.5)  # [0.5 / 1.5]
  # 4) Bimodal RWD + scale gap (RCT tight light-tailed, RWD wide bimodal).
  if (s == "4") return(if (rct) std_t(n, 8) * 0.5                         # [0.5 / ~1.6]
                       else mix_norm(n, c(-0.9, 0.9), c(0.5, 0.5), 0.4) * 1.6)
  # 5) Multimodal both, shared atoms, large weight AND scale gap (showcase).
  if (s == "5") return(if (rct)                                          # [~0.6 / ~1.7]
                         mix_norm(n, c(-2, 0, 2), c(0.15, 0.70, 0.15), 0.3) * 0.6
                       else
                         mix_norm(n, c(-2, 0, 2), c(0.45, 0.10, 0.45), 0.3) * 1.7)
  stop("unknown error_setting: ", s)
}

# Latent log-survival generators (no censoring), one per source.  With lambda = 0
# both sources share m0 + A*tau and differ only through the error term.
latent_rct <- function(n, setting) {
  X <- draw_X(n)
  A <- rbinom(n, 1L, 0.5)                              # randomised
  list(X = X, A = A,
       logT = m0(X) + A * tau(X) + draw_eps(n, setting, "rct"))
}
latent_rwd <- function(n, setting) {
  X <- draw_X(n)
  U <- runif(n)                                        # RWD-only (no effect, lu = 0)
  e <- plogis(X[, 1] + U)
  A <- rbinom(n, 1L, e)
  list(X = X, A = A, e = e,
       logT = m0(X) + A * tau(X) + draw_eps(n, setting, "rwd"))
}
...
calibrate <- function(setting, n_cal = 2e5) {
  rate_rct <- solve_cens_rate(latent_rct(n_cal, setting)$logT)
  visits   <- as.numeric(quantile(exp(latent_rwd(n_cal, setting)$logT),
                                  probs = visit_probs))
...
make_data <- function(n_rct, n_rwd, setting, rate_rct, visits) {
  r <- latent_rct(n_rct, setting)
  w <- latent_rwd(n_rwd, setting)
  ...
  list(
    ...
    # Mean and SD used to standardise the pooled latent log-survival.  Predicted
    # CATEs are rescaled by scale_sd; full-outcome predictions by scale_sd and
    # shifted by scale_mu, before evaluation on the log-time scale.
    scale_sd = scale_sd, scale_mu = scale_mu,
    n_rct = n_rct, n_rwd = n_rwd
  )
}

# ── Fit (FusionForest, log-time, interval-censoring args; error_dist varies) ──
fit_fusion <- function(d, N_post, N_burn, error_dist) {
  FusionForest_temp(
    y                            = d$y,
    status                       = d$status,
    ...
    X_test_treat              = d$X,
    X_test_deconf             = d$X,
    # Predict at the OBSERVED (A, S) so test_predictions is the full conditional
    # mean E[log T | X, A, S] (sum of forests) for each subject.  The treat-forest
    # prediction (the CATE) is A-independent, so CATE metrics are unaffected.
    treatment_indicator_test  = d$A,
    source_indicator_test     = d$S,
...
# Error-distribution priors compared (the HDPM protagonist plus two baselines).
error_priors <- c("gaussian", "shared_dp", "source_hdp_scale")

# Reconstruct the full-outcome posterior sample (draws x n) at the observed
# (A, S) from the stored component sample arrays, mirroring the backend's
# total = control + (1 - S) * deviation + A * (treat + (1 - S) * deconf).
# Returns NULL if any component sample is missing.
total_sample <- function(ff, A, S) {
  need <- c("test_predictions_sample_control", "test_predictions_sample_treat",
            "test_predictions_sample_deconf",  "test_predictions_sample_deviation")
  if (!all(need %in% names(ff))) return(NULL)
  oms <- 1 - S
  ff$test_predictions_sample_control +
    sweep(ff$test_predictions_sample_deviation, 2, oms, `*`) +
    sweep(ff$test_predictions_sample_treat +
            sweep(ff$test_predictions_sample_deconf, 2, oms, `*`), 2, A, `*`)
}

# ── One replication: the fusion forest under each error prior, one dataset ────
# For each prior we record two targets: the CATE tau(X) (treatment contrast) and
# the full conditional-mean outcome E[log T | X, A, S] (sum of forests).  The
# outcome target does not cancel the shared baseline m0, so it is the more
# sensitive probe of the residual model.
run_one_sim <- function(seed, n_rct, n_rwd, N_post, N_burn, setting,
                        rate_rct, visits) {
  set.seed(seed)
  d     <- make_data(n_rct, n_rwd, setting, rate_rct, visits)
  n_all <- n_rct + n_rwd
  safe  <- function(expr) tryCatch(expr, error = function(e) NULL)

  # Fits run on the standardised log scale: CATE = treat-forest * scale_sd; the
  # full outcome = total prediction * scale_sd + scale_mu.
  s  <- d$scale_sd; mu <- d$scale_mu; A <- d$A; S <- d$S
  truth_cate <- tau(d$X)
  truth_out  <- m0(d$X) + A * tau(d$X)   # full conditional mean at observed (A, S); lambda = 0

  res <- do.call(rbind, lapply(error_priors, function(ed) {
    ff <- safe(fit_fusion(d, N_post, N_burn, ed))
    # CATE (treatment-effect contrast; A-independent).
    rc <- make_rows(ed,
            if (!is.null(ff)) ff$test_predictions_treat * s,
            if (!is.null(ff)) ff$test_predictions_sample_treat * s,
            truth_cate, n_rct, n_all)
    rc$target <- "cate"
    # Full-outcome conditional mean (sum of forests) at observed (A, S).
    op <- os <- NULL
    if (!is.null(ff)) {
      ts <- total_sample(ff, A, S)
      if (!is.null(ts)) { op <- ff$test_predictions * s + mu; os <- ts * s + mu }
    }
    ro <- make_rows(ed, op, os, truth_out, n_rct, n_all)
    ro$target <- "outcome"
    rbind(rc, ro)
  }))
  ...
  res$rc_rwd_tail <- mean(d$icc_rwd == 0)           # RWD right-censored past last visit
  res
}

# ── Parallel runner over replications (one error setting) ────────────────────
run_simulation_tidy <- function(n_rep, n_rct, n_rwd, N_post, N_burn,
                                setting, seed_offset = 1000L) {
  cal <- calibrate(setting)       # per-setting RCT rate + RWD decile visits (fixed)
  ...
  foreach(
    ...
    res <- run_one_sim(seed_offset + i, n_rct, n_rwd, N_post, N_burn, setting,
...
cat("SIMULATION: error-distribution prior (sim_err_v2): FusionForest under",
    "gaussian / shared_dp / source_hdp_scale -- HPC\n")
cat("lambda_u = lambda_d = 0 (sources differ only in error distribution);",
    "per-source scale gap + heavy coarse censoring to stress the error model.\n")
cat("RWD interval-censored at quartiles q25/q50/q75; RCT right-censored ~50%.\n")
cat("Covariates: MVN AR(1), rho =", rho_X, "\n")

# Simulation settings.
M       <- 1000             # replications per setting (start small)
n_rct   <- 150
n_rwd   <- 350L
N_post  <- 5000L
N_burn  <- 5000L
cat(sprintf("M = %d reps/setting, n_rct = %d, n_rwd = %d, N_post = %d, N_burn = %d\n",
            M, n_rct, n_rwd, N_post, N_burn))

# Five error settings, easy (1) to hard (5).
settings <- 1:5
cat(sprintf("Settings: %s\n", paste(settings, collapse = ", ")))

all_cells <- vector("list", length(settings))
for (g in seq_along(settings)) {
  setting <- settings[g]
  cat(sprintf("\nSetting %d/%d (error_setting = %d)\n",
              g, length(settings), setting))
  all_cells[[g]] <- run_simulation_tidy(
    n_rep = M, n_rct = n_rct, n_rwd = n_rwd,
    N_post = N_post, N_burn = N_burn,
    setting = setting, seed_offset = 1000L * g)
}
final_flat_df <- do.call(rbind, all_cells)

# Output file path (! NAME MUST BE FILENAME_output.rds !).
output_file <- file.path(Sys.getenv("TMPDIR"), "sim_err_v2_output.rds")
...
# Reads sim_err_v2_output.rds and prints, per (setting, method, population):
#   rmse, bias, across-rep variance/sd of the integrated bias, coverage, postvar.
# Then draws (a) the four CATE metrics by error setting and (b) an illustrative
# recovery of the per-source error densities at the hardest setting.
# ──────────────────────────────────────────────────────────────────────────────
# res <- readRDS("simulations/exp4_error_distribution/sim_err_v2_output.rds")
# 
# prior_labs <- c(gaussian = "Gaussian", shared_dp = "Shared DP",
#                 source_hdp_scale = "Source HDP")
# prior_lvls <- names(prior_labs)
# prior_cols <- c(gaussian = "grey55", shared_dp = "goldenrod1",
#                 source_hdp_scale = "olivedrab")   # HDP = protagonist
# 
# grp <- cbind(rmse, bias, coverage, width, postvar) ~ setting + method + population + target
# mean_tbl <- aggregate(grp, data = res,
#                       FUN = function(x) mean(x, na.rm = TRUE), na.action = na.pass)
# var_tbl  <- aggregate(bias ~ setting + method + population + target,
...
#
# summary_tbl$method     <- factor(summary_tbl$method, levels = prior_lvls)
# summary_tbl$population  <- factor(summary_tbl$population, levels = c("RCT", "RWD", "All"))
# summary_tbl$target      <- factor(summary_tbl$target, levels = c("cate", "outcome"))
#
# # Two targets: the CATE tau(x) and the full conditional-mean outcome (sum of
# # forests).  The outcome target does not cancel the shared baseline m0, so it
# # is the more sensitive probe of the residual model.
# for (tg in c("cate", "outcome")) {
#   cat(sprintf("\n=== %s metrics by error setting (mean over replications) ===\n",
#               if (tg == "cate") "CATE" else "Outcome log(T)"))
#   sub <- summary_tbl[summary_tbl$target == tg, ]
#   o   <- order(sub$setting, sub$population, sub$method)
#   print(sub[o, c("setting", "population", "method",
#                  "rmse", "bias", "variance", "sd", "coverage", "width", "postvar")],
#         row.names = FALSE, digits = 3)
# }
# 
# # Realised censoring fractions per setting (sanity check):
# # cens_tbl <- aggregate(cbind(cens_rct, ic_rwd, rc_rwd_tail) ~ setting,
# #   data = res, FUN = function(x) mean(x, na.rm = TRUE))
# # print(cens_tbl, row.names = FALSE, digits = 3)
# 
# # Exploratory figures (not manuscript figures yet) go in a local sub-folder.
# fig_dir <- "simulations/exp4_error_distribution/figures"
...
# 
# # ── (a) The four CATE metrics by error setting, boxes grouped by setting with
# #        the three error priors side by side; one shared legend beneath.
# plot_metrics_by_setting <- function(dat, pop = "All", target = "outcome", file = NULL) {
#   dat <- dat[dat$population == pop & dat$target == target, ]
#   dat$method  <- factor(dat$method,  levels = prior_lvls)
#   dat$setting <- factor(dat$setting, levels = sort(unique(dat$setting)))
#   cols <- prior_cols[prior_lvls]
#   render <- function() {
#     op <- par(mfrow = c(2, 2), family = "serif", oma = c(5, 0, 0, 0),
#               mar = c(6.5, 7.5, 2, 2), mgp = c(4.3, 1.3, 0),
#               cex.lab = 2.4, cex.axis = 2.0)
#     nb <- length(prior_lvls); ng <- nlevels(dat$setting); gap <- 1.5
#     at      <- as.vector(outer(seq_len(nb), (seq_len(ng) - 1) * (nb + gap), "+"))
#     centers <- (seq_len(ng) - 1) * (nb + gap) + (nb + 1) / 2
#     seps    <- centers[-ng] + diff(centers) / 2
#     for (metric in metrics) {
#       ylim_m <- range(boxplot(dat[[metric]] ~ dat$method + dat$setting,
#                               plot = FALSE)$stats, na.rm = TRUE)
#       if (metric == "coverage") ylim_m <- range(c(ylim_m, 0.95))
#       if (metric == "bias")     ylim_m <- range(c(ylim_m, 0))
#       boxplot(dat[[metric]] ~ dat$method + dat$setting, col = cols, xaxt = "n",
#               at = at, xlim = range(at) + c(-0.5, 0.5), ylim = ylim_m,
#               outline = FALSE, main = "", ylab = metric_labs[metric],
#               xlab = "Error setting (easy to hard)", sep = " ")
#       axis(1, at = centers, labels = levels(dat$setting))
...
#     plot.new()
#     legend("bottom", legend = prior_labs[prior_lvls], fill = cols,
#            horiz = TRUE, bty = "n", xpd = TRUE, cex = 2.4)
#     par(op)
#   }
#   render()
#   if (!is.null(file)) {
...
#   }
# }
# plot_metrics_by_setting(res, pop = "All", target = "cate",
#                         file = file.path(fig_dir, "sim_err_metrics_cate.pdf"))
# plot_metrics_by_setting(res, pop = "All", target = "outcome",
#                         file = file.path(fig_dir, "sim_err_metrics_outcome.pdf"))
# 
# # ── (b) Error-density recovery at the hardest setting (illustrative).
# # Fits each prior once on a single setting-5 dataset and overlays the
# # posterior-mean fitted per-source residual density on the true error density
# # (both on the standardised residual scale; the truth is a large-sample KDE of
# # eps / scale_sd).
# #
# # NOTE: reconstruction reads the returned mixture fields per source group g:
# #   dp_mix_prop[[g]] (N_post x K weights), dp_locations[[g]] (N_post x K atoms),
# #   dp_sigma_g[[g]]  (N_post per-source sd; scale modes only).
# # It assumes dp_locations[[g]] are the per-source atoms entering the likelihood
# # (already centred), and group 1 = RCT, group 2 = RWD.  VERIFY this ordering and
# # centring against a real fit before trusting the figure; adjust `grp_of` if not.
# density_recovery <- function(setting = 5L, seed = 1L, file = NULL,
#                              grid = seq(-4, 4, length.out = 400)) {
#   set.seed(seed)
#   cal <- calibrate(setting)
#   d   <- make_data(150L, 350L, setting, cal$rate_rct, cal$visits)
#   sc  <- d$scale_sd
#   true_dens <- function(source) {                  # KDE of standardised true error
#     e <- draw_eps(2e5, setting, source) / sc
#     approxfun(density(e, n = 1024), rule = 2)
#   }
#   fd <- list(rct = true_dens("rct"), rwd = true_dens("rwd"))
# 
#   fitted_dens <- function(fit, g) {
#     if (is.null(fit)) return(rep(NA_real_, length(grid)))
#     if (is.null(fit$dp_mix_prop)) return(dnorm(grid, 0, mean(fit$sigma)))  # gaussian
#     W  <- fit$dp_mix_prop[[g]]; L <- fit$dp_locations[[g]]
#     sg <- if (!is.null(fit$dp_sigma_g)) fit$dp_sigma_g[[g]] else rep(mean(fit$sigma), nrow(W))
#     dens <- numeric(length(grid))
#     for (it in seq_len(nrow(W))) {
#       M <- outer(L[it, ], grid, function(mu, w) dnorm(w, mu, sg[it]))  # K x G
#       dens <- dens + colSums(W[it, ] * M)
#     }
#     dens / nrow(W)
#   }
#   grp_of <- function(fit, source) {
#     if (is.null(fit$dp_mix_prop) || length(fit$dp_mix_prop) == 1L) return(1L)  # pooled
#     if (source == "rct") 1L else 2L
#   }
# 
#   fits <- lapply(error_priors, function(ed)
#     tryCatch(fit_fusion(d, N_post = 2000L, N_burn = 2000L, ed),
#              error = function(e) NULL))
#   names(fits) <- error_priors
# 
#   render <- function() {
#     op <- par(mfrow = c(1, 2), family = "serif", oma = c(4, 0, 2, 0),
#               mar = c(5, 5, 3, 1), mgp = c(3, 1, 0), cex.lab = 1.6, cex.axis = 1.4)
#     for (source in c("rct", "rwd")) {
#       plot(grid, fd[[source]](grid), type = "l", lwd = 4, col = "black",
#            xlab = "standardised residual", ylab = "density",
#            main = if (source == "rct") "RCT source" else "RWD source")
#       for (ed in error_priors)
#         lines(grid, fitted_dens(fits[[ed]], grp_of(fits[[ed]], source)),
#               col = prior_cols[ed], lwd = 3)
#     }
#     par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0),
#         family = "serif", new = TRUE)
#     plot.new()
#     legend("bottom", legend = c("Truth", prior_labs[error_priors]),
#            col = c("black", prior_cols[error_priors]), lwd = c(4, 3, 3, 3),
#            horiz = TRUE, bty = "n", xpd = TRUE, cex = 1.4)
#     par(op)
#   }
#   render()
#   if (!is.null(file)) {
#     pdf(file, width = 14, height = 6, family = "Times")
#     render(); dev.off()
#   }
# }
# density_recovery(setting = 5L, seed = 1L,
#                  file = file.path(fig_dir, "sim_err_density.pdf"))
# 
