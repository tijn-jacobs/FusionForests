# ============================================================================
# IRS Benchmark v2 — MICE + BART only
#
# Usage:
#   Rscript irs_v2_mice.R [num_cores]
#
# Output:
#   $TMPDIR/irs_v2_mice_output.rds
# ============================================================================
...
run_mice <- function(data, miss, s, m_imp = 5) {
  ...
  fit_1 <- list(
    train_predictions = pool_pred("train_predictions",
                                  "fit_1"),
    test_predictions  = pool_pred("test_predictions",
                                  "fit_1")
  )
  fit_0 <- list(
    train_predictions = pool_pred("train_predictions",
...
for (scenario_id in seq_len(n_scenarios)) {
  ...
  results <- foreach(
    rep = seq_len(n_reps),
    .combine  = rbind,
    .packages = c("FusionForests", "MASS", "mice"),
    .errorhandling = "stop"
  ) %dopar% {
    ...
    make_row("MICE+BART", ev, (proc.time() - t0)[3])
  }
