################################################################################
# FusionForest prior-specification simulation — UNCENSORED variant, v2 (HPC).
#
# Follow-up to simulation_uncensored_hpc.R. The v1 study identified Robust
# (k_d, k_tau, k_c) = (2, 1.7, 2) as the across-the-board winner but flagged
# substantial over-coverage (>= 0.94 in every non-Efficiency cell), suggesting
# that the total variance budget omega^2 = 1 on standardised log T is too
# generous. v2 fine-tunes two knobs around the v1 winner:
#
#   * k_tau in {1.5, 1.7, 2.0}, holding k_d = k_c = 2 fixed.
#   * omega^2 in {0.95, 1.00}, scaling the total prior variance budget.
#
# omega enters via to_code_ks() as s0 = omega / sqrt(1 + sum_f k_f^{-2}).
# A smaller omega tightens every forest's leaf-prior SD proportionally.
#
# Conventions:
#   * Cores read from CLI arg (e.g. `Rscript simulation_uncensored_hpc_v2.R 192`).
...
#     fits all six v2 configs serially on one core.
...
# omega scales the total prior variance budget: sum_f Var_pi(f) = omega^2.
# Internally s0 = omega / sqrt(1 + sum_f k_f^{-2}) with k_sh_setup = 1.
...
# v2 grid: k_tau in {1.5, 1.7, 2.0} crossed with omega in {0.95, 1.00}.
# k_d = k_c = 2 throughout; depths, trees, and DGP unchanged from v1.
build_configs <- function() {
  ...
  cfg <- function(label, k_d, k_tau, k_c, omega, depth = base_depth) {
  ...
  list(
    kt15_o100 = cfg("kt15_o100", 2.0, 1.5, 2.0, 1.00),
    kt17_o100 = cfg("kt17_o100", 2.0, 1.7, 2.0, 1.00),
    kt20_o100 = cfg("kt20_o100", 2.0, 2.0, 2.0, 1.00),
    kt15_o095 = cfg("kt15_o095", 2.0, 1.5, 2.0, 0.95),
    kt17_o095 = cfg("kt17_o095", 2.0, 1.7, 2.0, 0.95),
    kt20_o095 = cfg("kt20_o095", 2.0, 2.0, 2.0, 0.95)
  )
}
...
cat("SIMULATION: FusionForest prior spec, v2 (UNCENSORED) — HPC\n")

set.seed(42)

M               <- 1000
n_pairs         <- list(c(100L, 400), c(400, 100L))
...
# Place this script and simulation_uncensored_hpc_v2_output.rds in the same
...
# ──────────────────────────────────────────────────────────────────────────────
# res <- readRDS("simulations/prior/simulation_uncensored_hpc_v2_output.rds")
# 
