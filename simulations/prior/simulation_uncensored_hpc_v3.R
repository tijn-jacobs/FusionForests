################################################################################
# FusionForest prior-specification simulation — UNCENSORED variant, v3 (HPC).
#
# Tree-count sensitivity sweep around the v2 anchor (k_d, k_tau, k_c) =
# (2, 1.5, 2), omega = 1.0, sigma = 1.0.  Holds those four hyperparameters
# fixed and varies the tree counts of the treatment-effect and confounding
# forests independently:
#
#   * m_sh = 200       (baseline, fixed)
#   * m_d  = 50        (deviation, fixed; auxiliary cross-source forest)
#   * m_tau in {50, 100, 200}
#   * m_c   in {50, 100, 200}
#
# That gives 3 x 3 = 9 configurations.  The DGP, depth priors, MCMC budget,
# and grid (4 scenarios x 2 CATE regimes x 2 sample-size pairs) are
# unchanged from v1/v2.
#
# Conventions:
#   * Cores read from CLI arg (e.g. `Rscript simulation_uncensored_hpc_v3.R 192`).
...
#     fits all nine v3 configs serially on one core.
...
# v3 fixes (k_d, k_tau, k_c) = (2, 1.5, 2), omega = 1.  Trees for the
# treatment and confounding forests are swept; baseline and deviation are
# held fixed.
K_D     <- 2.0
K_TAU   <- 1.5
K_C     <- 2.0
OMEGA   <- 1.0
M_TAU_GRID <- c(50L, 100L, 200L)
M_C_GRID   <- c(50L, 100L, 200L)
M_SH       <- 200L
M_D        <- 50L
...
build_configs <- function() {
  ...
  cfg <- function(m_tau, m_c) {
    label <- sprintf("t%d_c%d", m_tau, m_c)
    c(list(label     = label,
  ...
  cells <- expand.grid(m_tau = M_TAU_GRID, m_c = M_C_GRID,
                       KEEP.OUT.ATTRS = FALSE)
  out <- vector("list", nrow(cells))
  for (i in seq_len(nrow(cells))) {
    entry <- cfg(cells$m_tau[i], cells$m_c[i])
...
cat("SIMULATION: FusionForest prior spec, v3 (UNCENSORED) — HPC\n")

set.seed(42)

M               <- num_cores
n_pairs         <- list(c(200L, 300L))
...
# Place this script and simulation_uncensored_hpc_v3_output.rds in the same
...
#   2. m_tau x m_c heatmap of RMSE at the central cell (S2, het, (250, 1000))
# ──────────────────────────────────────────────────────────────────────────────
# res <- readRDS("simulations/prior/simulation_uncensored_hpc_v3_output.rds")
# 
